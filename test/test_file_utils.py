...
def test_search_for_file():
    ...
    with temporary_directory() as dirname:
        ...
        with open(filepath := os.path.join(dirname, filename), "w"):
            with open(subfilepath := os.path.join(subdirname, filename), "w"):
                assert set(search_for_file(filename, location=dirname, recursive=True)) == set([filepath, subfilepath])
    ...
    with temporary_directory() as dirname:
        ...
        with open(filepath := os.path.join(dirname, filename), "w"):
            with open(subfilepath := os.path.join(subdirname, filename), "w"):
                assert set(search_for_file(filename, location=topdirname, recursive=True)) == set([filepath, subfilepath])
