...
def _test_boto_monkey_patching_client_without_overriding(service: str):
    with override_environ(**{_boto_monkey_patching_endpoint_url_environ_name(service): None}):
...
def _test_boto_monkey_patching_client_with_overriding(service: str):
    with override_environ(**{_boto_monkey_patching_endpoint_url_environ_name(service): _override_endpoint_url}):
        s3_client = boto3.client(service)
